...
class MeanClusteringError(nn.Module):
    ...
    def __init__(self, num_classes,total,lama, tau=1):
        super(MeanClusteringError, self).__init__()
        self.register_buffer('embedding', torch.eye(num_classes))
        self.tau = tau
        self.num_classes = num_classes
        self.USE_CUDA = torch.cuda.is_available()
        self.target = torch.zeros(total, self.num_classes).cuda() if self.USE_CUDA else torch.zeros(total, self.num_classes)
        self.target1 = torch.zeros(total, self.num_classes).cuda() if self.USE_CUDA else torch.zeros(total, self.num_classes)
    ...
    def forward(self,input, target, index=-1,type=1, threshold=1):
        ...
        elr_reg=0
        if index!=-1:
            y_pred = torch.clamp(pred, 1e-4, 1.0-1e-4)
            y_pred_ = y_pred.data.detach()
            if type==1:
                self.target[index] = self.beta * self.target[index] + (1-self.beta) * ((y_pred_)/(y_pred_).sum(dim=1,keepdim=True))
                elr_reg = ((1-(self.target[index] * y_pred).sum(dim=1)).log()).mean()
            else:
                self.target1[index] = self.beta * self.target1[index] + (1 - self.beta) * (
                            (y_pred_) / (y_pred_).sum(dim=1, keepdim=True))
                elr_reg = ((1 - (self.target1[index] * y_pred).sum(dim=1)).log()).mean()

        return  (p.log()).mean()+self.lama*elr_reg
