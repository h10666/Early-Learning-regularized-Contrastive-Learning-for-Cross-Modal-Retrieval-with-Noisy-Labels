...
def main():
    ...
    if args.ls == 'cos':
        lr_schedu = optim.lr_scheduler.CosineAnnealingLR(optimizer, 100, eta_min=0, last_epoch=-1)
    else:
    ...
    if args.loss == 'CE':
        ...
        criterion = utils.MeanClusteringError(train_dataset.class_num,total,lama=args.lama, tau=args.tau).cuda()
    else:
    ...
    def cross_modal_contrastive_ctriterion(fea,pred, tau=1.):

        batch_size = fea[0].shape[0]
        pred = [item.argmax(dim=1) for item in pred]
        # 模态内对比学习
        sim_IMG = fea[0].mm(fea[0].t())
        sim_IMG = (sim_IMG / tau).exp()
        sim_IMG = sim_IMG - sim_IMG.diag().diag()
        sum_sim_IMG = sim_IMG.sum(1)
        labels_IMG = pred[0].expand(batch_size, batch_size).eq(
            pred[0].expand(batch_size, batch_size).t()).float() - 2 * torch.eye(batch_size, batch_size).cuda()
        loss_img_se = 0
        for i in range(batch_size):
            label_IMG = labels_IMG[i]
            pos_inds = torch.nonzero(label_IMG == 1).squeeze(1)
            loss_img_se += -(sim_IMG[i, pos_inds] / sum_sim_IMG[i]).log().sum() / (2 * len(pos_inds) - 1)


        sim_txt = fea[1].mm(fea[1].t())
        sim_txt = (sim_txt / tau).exp()
        sim_txt = sim_txt - sim_txt.diag().diag()
        sum_sim_txt = sim_txt.sum(1)
        labels_txt = pred[1].expand(batch_size, batch_size).eq(
            pred[1].expand(batch_size, batch_size).t()).float() - 2 * torch.eye(batch_size, batch_size).cuda()
        loss_txt_se = 0
        for i in range(batch_size):
            label_txt = labels_txt[i]
            pos_inds = torch.nonzero(label_txt == 1).squeeze(1)
            loss_txt_se += -(sim_txt[i, pos_inds] / sum_sim_txt[i]).log().sum() / (2 * len(pos_inds) - 1)
        #模态间对比学习
        ...
        return (loss_img_se + loss_txt_se) * args.zeta+loss1+loss2
    ...
    def train(epoch):
        ...
        for batch_idx, (batches, targets, index) in enumerate(train_loader):
            ...
            losses = [criterion(preds[v], targets[v],index.data.cpu().numpy().tolist(),v) for v in range(n_view)]#早期正则化
            loss = sum(losses)
            beta = sigmoid_rampup(count_epoch, args.betasetp) * args.beta
            loss = beta * loss + (1. - beta) * cross_modal_contrastive_ctriterion(outputs,preds, tau=1)
            if epoch >= 0:
                loss.backward()
                optimizer.step()
                if local==0 and epoch==0:
                    for img_par in multi_models[0].parameters():
                        img_par.data.mul_(args.alpha)
                    for txt_par in multi_models[1].parameters():
                        txt_par.data.mul_(args.alpha)
                    multi_models[0].img_par_list = multi_models[0].parameters()
                    multi_models[1].txt_par_list = multi_models[1].parameters()
                else:
                    #子网络权重平均
                    alpha = sigmoid_rampup(epoch + local, args.setp) * args.alpha#zeta1
                    for img_par, old_img_par in zip(multi_models[0].parameters(), multi_models[0].img_par_list):
                        img_par.data.mul_(alpha).add_(1 - alpha, old_img_par.data)
                    for txt_par, old_txt_par in zip(multi_models[1].parameters(), multi_models[1].txt_par_list):
                        txt_par.data.mul_(alpha).add_(1 - alpha, old_txt_par.data)
                    multi_models[0].img_par_list=multi_models[0].parameters()
                    multi_models[1].txt_par_list=multi_models[1].parameters()
    ...
    def eval(data_loader, epoch, mode='test'):
            ...
            if sum([data_loader.dataset.train_data[v].shape[0] != data_loader.dataset.train_data[0].shape[0] for v in range(len(data_loader.dataset.train_data))]) == 0:
                for batch_idx, (batches, targets, index) in enumerate(data_loader):
                    ...
                    for v in range(n_view):
                        ...
                        losses.append(criterion(pred[v], targets[v],index.data.cpu().numpy().tolist(),v))
                        loss_list[v] += losses[v]
        ...
        print(('Accuracy:' + mode+':img----{:.2f},txt-----{:.2f}').format(correct_list[0] / total_list[0],correct_list[1] / total_list[1]))
        return fea, lab
